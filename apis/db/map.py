...
def get_dvr_coords(auth_id):
    con = mysql.connector.connect(
        host="localhost", user="root", password="", database="xyz"
    )
    cursor = con.cursor()
    # if data["areas"][0] == "*":
    query2 = f"select dvr_id,status,loc,ownership,associated_cams_id,associated_cams_loc,associated_cams_status from dvr where auth_id = '{auth_id}'"
