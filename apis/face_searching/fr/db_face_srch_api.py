...
def initiate_face_srch(auth_id: str, date: str, init_time: str, end_time: str):
    camera_url_list = []
    face_img_list = []
    cam_id_list = []
    response = []
    ...
    cursor = con.cursor()
    if filter_by == "area":
        query2 = f"select video_id,cam_id, from camera where auth_id = '{auth_id}' AND date = '{filter_data}'"
    else:
